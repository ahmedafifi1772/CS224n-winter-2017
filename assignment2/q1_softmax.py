...
def softmax(x):
    ...
    out = tf.placeholder(tf.float64, x.shape)

    maxx = tf.reduce_max( x, axis=1 )
    out = tf.transpose( tf.transpose(x) - maxx  ) 
    
    out = tf.exp(out)
    
    out = tf.div( out, tf.expand_dims( tf.reduce_sum( out, axis =1) , 1)  )
    
#    print("out", tf.reduce_sum( out, axis =1).eval(), out.eval() )
...
def cross_entropy_loss(y, yhat):
    ...
    out = tf.placeholder(tf.float32, shape=[1,])
#    print( "outt", tf.reduce_sum(tf.multiply( tf.to_float(y), tf.log(yhat)).eval() ) )
