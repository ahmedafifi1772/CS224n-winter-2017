...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings_tensors = tf.Variable( self.pretrained_embeddings,
                                         [None, self.config.n_features, self.config.embed_size])
        embeddings = tf.nn.embedding_lookup( embeddings_tensors, self.input_placeholder)
        
        embeddings=tf.reshape(tensor = embeddings, shape=[-1, (self.config.n_features * self.config.embed_size) ])
    ...
    def add_prediction_op(self):
        ...
        x = self.add_embedding() #(None, n_features*embed_size)
        ### YOUR CODE HERE
        xavier_initializer = xavier_weight_init()
        W= xavier_initializer(shape=[  (self.config.n_features * self.config.embed_size),
    ...
    def add_loss_op(self, pred):
        ...
        # labels & logits are of same size [ batch_size, n_classes]
        # this should return 1-D tensor of size [batch_size] same dtype as logits
