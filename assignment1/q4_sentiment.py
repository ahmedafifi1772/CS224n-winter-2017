...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sumSentenceWordVecs = [ 0 for x in xrange(wordVectors.shape[1]) ]
    for word in sentence:
        wordVec = wordVectors[ tokens[word] ]
        sumSentenceWordVecs += wordVec
    
    sentVector = sumSentenceWordVecs/ len( sentence )
    ### END YOUR CODE
    
    # same num of col ( word vector size = sentnce victor size)
...
def chooseBestModel(results):
    ...
    for model in results:
        if (model['dev'] > bestResult['dev']) & (model['test'] > bestResult['test']):
            bestResult = model
