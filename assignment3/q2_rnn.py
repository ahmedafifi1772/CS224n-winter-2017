...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        for i in range(max_length):
            newSen.append( sentence[i] if ( i< len(sentence) ) else zero_vector)            
            newLab.append( labels[i] if ( i < len(labels)) else zero_label)
            mask.append( True if (i < len(sentence)) else False)
        ret.append ( (newSen, newLab, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder= tf.placeholder( shape=[None, self.max_length], dtype=tf.int32)
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed_tensor = tf.Variable( self.pretrained_embeddings, [self.config.n_features, self.config.embed_size])
        embeddings= tf.nn.embedding_lookup(embed_tensor, self.input_placeholder)
        # u need -1 trick in case u have unknown ( changable ) shape vector , which means u do reshape and let -1 handle None part
    ...
    def add_prediction_op(self):
        ...
        h_dim = tf.stack([ tf.shape(x)[0], self.config.hidden_size])
        h = tf.zeros(shape= h_dim)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        cross_entorpy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels= tf.boolean_mask(self.labels_placeholder), logits=  tf.boolean_mask(pred) )
