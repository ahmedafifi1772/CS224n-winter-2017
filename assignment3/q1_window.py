...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for x in range( len(sentence)):            
            ...
            for sh in range(1, window_size+1):
                #left
                if( x-sh) <0:
                    left.extend(start)
                else:
                    left.extend(sentence[x-sh])
                    
                #right
                if( x+sh) >= (len(sentence) ):
                    right.extend(end)
                else:
                    right.extend( sentence[x+sh] )
            
            left.extend(sentence[x])
            left.extend(right)

        #end of word window fox x index word in sentence
        
            windowed_data.append( (left, labels[x]) )
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings_tensor= tf.Variable(self.pretrained_embeddings,
                                [None, self.config.n_window_features, self.config.embed_size])
...
#        W = tf.contrib.layers.xavier_initializer(shape=[self.config.n_window_features* self.config.embed_size, self.config.hidden_size ])
        b1 =tf.get_variable("b1", shape=[self.config.hidden_size]
                        , initializer= tf.contrib.layers.xavier_initializer() )
#        b1 = tf.contrib.layers.xavier_initializer(shape=[self.config.hidden_size])
        U  = tf.get_variable("U", shape=[self.config.hidden_size, self.config.n_classes]
                        , initializer= tf.contrib.layers.xavier_initializer() )
#        U  = tf.contrib.layers.xavier_initializer(shape=[self.config.hidden_size, self.config.n_classes])
        b2 =tf.get_variable("b2", shape=[self.config.n_classes],
                            initializer= tf.contrib.layers.xavier_initializer() )
#        b2 =tf.contrib.layers.xavier_initializer(shape=[self.config.n_classes])
        
#        w ( 200 * 200),  x ( [ ? , 200] ) =========> h [200, 200]
        h = tf.nn.relu( tf.matmul(x,W) + b1)
        h_drop= tf.nn.dropout(h, self.config.dropout)
        pred= tf.matmul(h_drop, U) + b2
        ### END YOUR CODE
        return pred ############# [200, 5] 5 classes vectors with 200 vector size
    ...
    def add_loss_op(self, pred):
        ...
        cross_entorpy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits= pred)
